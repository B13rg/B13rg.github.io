...
DTN provides a few different delivery options what applications can select when transmitting an ADU.
These can be selected by the application to manage transfer and delivery of bundles.
Some options also provide diagnostic and security capabilities.


| Name                                           | Description                                                                                                                                                                                   | Hop DTN Node Actions                                                                                        | Destination DTN Node Actions                     |
| ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| Custody Transfer Requested                     | Requests sent bundles be delivered with enhanced reliability using custody transfer procedures. Custody is described in more detail in [Section 3.10](#310-reliability-and-custody-transfer). | Transmit bundles using reliable transfer protocols; responsibility for delivery may shift among custodians. | Final delivery handled through custodians.       |
| Source Node Custody Acceptance Required        | Requires the source DTN node to provide custody transfer for the sent bundles.                                                                                                                | Provide custody transfer to next hop; transmission fails if unavailable.                                    | N/A                                              |
| Report When Bundle Delivered                   | Requests a (single) Bundle Delivery Status Report be generated when the subject ADU is delivered to its intended recipient(s). This request is also known as "return-receipt".                | N/A                                                                                                         | Generate report upon delivery.                   |
| Report When Bundle Acknowledged by Application | Requests an Acknowledgement Status Report be generated when the subject ADU is acknowledged by a receiving application.                                                                       | N/A                                                                                                         | Generate report upon application acknowledgment. |
| Report When Bundle Received                    | Requests a Bundle Reception Status Report be generated when each sent bundle arrives at a DTN node. This is designed primarily for diagnostic purposes.                                       | Generate reception report upon arrival                                                                      | Generate report upon arrival at a DTN node.      |
| Report When Bundle Custody Accepted            | Requests a Custody Acceptance Status Report be generated when each sent bundle has been accepted using custody transfer. This is designed primarily for diagnostic purposes.                  | Generate custody acceptance status report upon bundle custody transfer                                      | Generate report upon custody acceptance.         |
| Report When Bundle Forwarded                   | Requests a Bundle Forwarding Status Report be generated when each sent bundle departs a DTN node after forwarding. This is designed primarily for diagnostic purposes.                        | Generate forwarding report upon forwarding bundle                                                           | N/A                                              |
| Report When Bundle Deleted                     | Requests a Bundle Deletion Status Report be generated when each sent bundle is deleted at a DTN node. This is designed primarily for diagnostic purposes.                                     | Generate deletion report upon deleting a bundle                                                             | Generate report upon deletion at a DTN node.     |
|                                                | Security-specific options enabled via [DTNSEC](https://datatracker.ietf.org/doc/html/rfc4838#ref-DTNSEC)                                                                                      |                                                                                                             |                                                  |
| Confidentiality Required                       | Requires the subject ADU be made secret from parties other than the source and the members of the destination EID.                                                                            | N/A                                                                                                         | Decrypt ADU                                      |
| Authentication Required                        | Requires all non-mutable fields in the bundle blocks be made cryptographically verifiable. This protects several fields, including the source and destination EIDs and the bundle's data.     | Verify authenticity of bundle                                                                               | ""                                               |
| Error Detection Required                       | Requires modifications to the non-mutable fields of each sent bundle be made detectable with high probability at each destination.                                                            | Detect errors in non-mutable fields at destination.                                                         | ""                                               |


#### 3.6.2 Administrative Records: Bundle Status Reports and Custody Signals

In most cases when a report is generated, it will be transmitted back to the bundle's `Report-to` EID.
Most of the reports are used for communicating diagnostics, except for the "Bundle Delivery" and "Acknowledged by application" reports which are used for end-to-end tracking and application-level feedback.

| Report Type                 | Source            | Content                                                                                                           | Destination                         | Generation Policy                                  | Related Delivery Option                          | Purpose                    |
| --------------------------- | ----------------- | ----------------------------------------------------------------------------------------------------------------- | ----------------------------------- | -------------------------------------------------- | ------------------------------------------------ | -------------------------- |
| Bundle Reception            | DTN Node          | Sent when a bundle arrives at a DTN node.                                                                         | Report-to EID (configured for BSRs) | Limited by local policy                            | `Report When Bundle Received`                    | Diagnostic                 |
| Custody Acceptance          | DTN Node          | Sent when a node accepts custody of a bundle with the Custody Transfer Requested option set.                      | Report-to EID                       | Limited by local policy                            | `Report When Bundle Custody Accepted`            | Diagnostic                 |
| Bundle Forwarded            | DTN Node          | Sent when a bundle departs after being forwarded, if the "Report When Bundle Forwarded" option is set.            | Report-to EID                       | Limited by local policy                            | `Report When Bundle Forwarded`                   | Diagnostic                 |
| Bundle Deletion             | DTN Node          | Sent when a bundle is discarded (e.g., due to expiration).                                                        | Report-to EID                       | Required if deletion is performed by the custodian | `Report When Bundle Deleted`                     | Diagnostic                 |
| Bundle Delivery             | Destination Node  | Sent by the destination node upon successful delivery of a bundle with the "Report When Bundle Delivered" option. | Report-to EID                       | Required for final delivery                        | `Report When Bundle Delivered`                   | End-to-end status tracking |
| Acknowledged by application | Destination Node  | Sent when an application acknowledges receipt of a bundle with the "Application Acknowledgment" option.           | Report-to EID                       | Requires explicit application action               | `Report When Bundle Acknowledged by Application` | Application-layer feedback |
| Custody Signal              | Current Custodian | Indicates successful or failed custody transfer (Boolean).                                                        | Current custodian's EID             | Mandatory for custody transfer status              | Custody Mgmt.                                    | Custody transfer tracking  |
...
stateDiagram-v2
  ...
  state BundleM {
    R : Receive bundle
    S : Store Bundle
    I : Initiate Custody Transfer
    Rm : Remove
    C : Create Custody State
    L : Transfer Bundle
    ...
    L --> Rm : Transfer Succeeds
    L --> S : Transfer Fails
    L --> I : Custody Requested
    I --> L : Custody Transfer Result
    Rm --> [*]
  }
  BundleM: Bundle Management
```
